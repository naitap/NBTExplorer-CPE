    ...
    public class RuleTreeController
    {
        ...
        public void CreateNode (TreeNode node, TagType type)
        {
            ...
            switch (type) {
                case TagType.TAG_BYTE:
                    newNode = CreateIntegralNode<ByteTagRule, TagNodeByte>("字节 (Byte)");
                    break;
                case TagType.TAG_SHORT:
                    newNode = CreateIntegralNode<ShortTagRule, TagNodeShort>("短整数 (Short)");
                    break;
                case TagType.TAG_INT:
                    newNode = CreateIntegralNode<IntTagRule, TagNodeInt>("整数 (Int)");
                    break;
                case TagType.TAG_LONG:
                    newNode = CreateIntegralNode<LongTagRule, TagNodeLong>("长整数 (Long)");
                    break;
                case TagType.TAG_FLOAT:
                    newNode = CreateFloatNode<FloatTagRule, TagNodeFloat>("单精度浮点数 (Float)");
                    break;
                case TagType.TAG_DOUBLE:
                    newNode = CreateFloatNode<DoubleTagRule, TagNodeDouble>("双精度浮点数 (Double)");
                    break;
                case TagType.TAG_STRING:
                    newNode = CreateStringNode("字符串 (String)");
                    break;
            }
        ...
        public void EditNode (TreeNode node)
        {
            ...
            if (rule is ByteTagRule)
                EditIntegralNode<ByteTagRule, TagNodeByte>(node, rule as ByteTagRule, "字节 (Byte)");
            else if (rule is ShortTagRule)
                EditIntegralNode<ShortTagRule, TagNodeShort>(node, rule as ShortTagRule, "短整数 (Short)");
            else if (rule is IntTagRule)
                EditIntegralNode<IntTagRule, TagNodeInt>(node, rule as IntTagRule, "整数 (Int)");
            else if (rule is LongTagRule)
                EditIntegralNode<LongTagRule, TagNodeLong>(node, rule as LongTagRule, "长整数 (Long)");
            else if (rule is FloatTagRule)
                EditFloatNode<FloatTagRule, TagNodeFloat>(node, rule as FloatTagRule, "单精度浮点数 (Float)");
            else if (rule is DoubleTagRule)
                EditFloatNode<DoubleTagRule, TagNodeDouble>(node, rule as DoubleTagRule, "双精度浮点数 (Double)");
            else if (rule is StringTagRule)
                EditStringNode(node, rule as StringTagRule, "字符串 (String)");
            else if (rule is WildcardRule)
                EditWildcardNode(node, rule as WildcardRule, "通配符");
        }
        ...
        public void CreateWildcardNode (TreeNode node)
        {
            ...
            TreeNode newNode = CreateWildcardNode("通配符");
