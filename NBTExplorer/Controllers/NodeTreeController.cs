...
namespace NBTExplorer.Controllers
{
    ...
    public class NodeTreeController
    {
        ...
        public ContextMenuStrip BuildNodeContextMenu (TreeNode frontNode, DataNode node)
        {
            ...
            if (node.HasUnexpandedChildren || node.Nodes.Count > 0) {
                if (frontNode.IsExpanded) {
                    ToolStripMenuItem itemCollapse = new ToolStripMenuItem("折叠(&C)", null, _contextCollapse_Click);
                    itemCollapse.Font = new System.Drawing.Font(itemCollapse.Font, System.Drawing.FontStyle.Bold);

                    ToolStripMenuItem itemExpandChildren = new ToolStripMenuItem("展开子项(&H)", null, _contextExpandChildren_Click);
                    ToolStripMenuItem itemExpandTree = new ToolStripMenuItem("展开树(&T)", null, _contextExpandTree_Click);
                    ...
                    ToolStripMenuItem itemExpand = new ToolStripMenuItem("展开(&E)", null, _contextExpand_Click);
            ...
            if (node.CanReoderNode) {
                ToolStripMenuItem itemUp = new ToolStripMenuItem("上移(&U)", Properties.Resources.ArrowUp, _contextMoveUp_Click);
                ToolStripMenuItem itemDn = new ToolStripMenuItem("下移(&D)", Properties.Resources.ArrowDown, _contextMoveDown_Click);
            ...
            if (node is DirectoryDataNode) {
                ...
                ToolStripMenuItem itemOpenExplorer = new ToolStripMenuItem("在任务资源管理器中打开(&E)", null, _contextOpenInExplorer_Click);
