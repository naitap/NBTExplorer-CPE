...
namespace NBTExplorer.Windows
{
    ...
    public partial class MainForm : Form
    {
        ...
        {
            if (!ConfirmAction("确定要打开新文件吗?"))
                return;

            using (OpenFileDialog ofd = new OpenFileDialog() {
                RestoreDirectory = true,
                Multiselect = true,
                Filter = "所有文件|*|NBT文件 (*.dat, *.schematic)|*.dat;*.nbt;*.schematic|区域文件 (*.mca, *.mcr)|*.mca;*.mcr",
                FilterIndex = 0,
            }) {
        ...
        {
            if (!ConfirmAction("确定要打开Minecraft存档文件夹吗?"))
                return;
            ...
            try {
                ...
                MessageBox.Show("无法打开默认的Minecraft存档目录");
                ...
                try {
                    ...
                    MessageBox.Show("无法打开当前目录，此工具可能与您的平台不兼容.");
                    Console.WriteLine(e.Message);
                    Application.Exit();
                }
            }
        ...
        private bool ConfirmExit ()
        {
            if (_controller.CheckModifications()) {
                if (MessageBox.Show("您当前有未保存的更改.  放弃更改并退出?", "未保存的更改", MessageBoxButtons.OKCancel) != DialogResult.OK)
                    return false;
            }
        ...
        {
            if (_controller.CheckModifications()) {
                if (MessageBox.Show("您当前有未保存的更改.  " + actionMessage, "未保存的更改", MessageBoxButtons.OKCancel) != DialogResult.OK)
                    return false;
            }
        ...
        {
            if (_controller.CheckModifications()) {
                if (MessageBox.Show("您当前有未保存的更改.  " + e.Message, "未保存的更改", MessageBoxButtons.OKCancel) != DialogResult.OK)
                    e.Cancel = true;
            }
        }
        ...
        {
            if (_nodeTree.SelectedNode == null || _nodeTree.SelectedNode.Tag == null)
            {
                MessageBox.Show("请先选择一个节点进行查找和替换操作.", "选择节点", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
        ...
        private void findBlockToolStripMenuItem_Click (object sender, EventArgs e)
        {
            if (_nodeTree.SelectedNode == null || _nodeTree.SelectedNode.Tag == null)
            {
                MessageBox.Show("请先选择一个节点进行查找区块操作.", "选择节点", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
        ...
        private void _cleanLogFile_Click(object sender, EventArgs e)
        {
            if (!Directory.Exists(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "NBTExplorer")))
            {
                MessageBox.Show("日志文件已清理完成.", "清理日志", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }

            if (MessageBox.Show("确定要清理错误日志文件吗? 此操作不可逆转", "确认清理", MessageBoxButtons.OKCancel) != DialogResult.OK)
                return;
            
            Console.Write("Delete File: " + Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "NBTExplorer"));
            
            Directory.Delete(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "NBTExplorer"), true);
            MessageBox.Show("日志文件已清理完成.", "清理完成", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }

        private void _openLogFile_Click(object sender, EventArgs e)
        {
            if (!Directory.Exists(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "NBTExplorer"))) {
                MessageBox.Show("没有找到日志文件.", "打开日志", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            Process.Start("notepad.exe", Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "NBTExplorer") + "/error.log");
        }
    }
}
